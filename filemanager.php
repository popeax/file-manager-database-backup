...
$wp_core= dirname(dirname(dirname(dirname(__FILE__)))).'/wp-load.php';
if (file_exists($wp_core))
{
	require_once($wp_core);
	$randomnum=get_option('myfmg_random_numb');
	if (empty($randomnum)) {update_option('myfmg_random_numb',rand(1,111111)*rand(1,1111111)); header("location:" . $_SERVER['REQUES_URI']) or die("cant_redirect_error_7955_inside_plugin");}
	global $current_user;
	$user_info= get_userdata( $current_user->ID ); //http://codex.wordpress.org/Function_Reference/get_userdata
	$lvl=$user_info->user_level;	 
	if ($lvl == '10' || $lvl == '9' || $lvl == '8')	
	{
		$ps			 =$randomnum;
		$_POST['pwd']=$ps;
		$_POST['tz_offset']= isset($_SESSION['tz_offset']) ? $_SESSION['tz_offset'] : 0;
	}
	else {	die('you are not admin'); }
}
else {	die('no_access. cant find wp-core.php'); }
